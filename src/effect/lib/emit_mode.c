...
EffectTri *effectEmitMesh(EffectPrimBuild *s, u32 *ot, s32 otShift,
    ...
    g_effectDrawMode = 0xE1000220;
    gte_SetFarColor(s->r, s->g, s->b);
    if (*s->cursor != 0) {
        prim = effectEmitTris(s, ot, otShift, prim);
    } else {
        s->cursor++;
    }
    if (*s->cursor != 0) {
        prim = effectEmitQuads(s, ot, otShift, prim);
    } else {
        s->cursor++;
    }
    if (*s->cursor != 0) {
        prim = effectEmitTexTris(s, ot, otShift, prim);
    } else {
        s->cursor++;
    }
    if (*s->cursor != 0) {
        prim = effectEmitTexQuads(s, ot, otShift, prim);
    } else {
        s->cursor++;
    }
    if (*s->cursor != 0) {
        prim = effectEmitGouraudTris(s, ot, otShift, prim);
    } else {
        s->cursor++;
    }
    if (*s->cursor != 0) {
        prim = effectEmitGouraudQuads(s, ot, otShift, prim);
    } else {
        s->cursor++;
    }
    if (*s->cursor != 0) {
        prim = effectEmitGouraudTexTris(s, ot, otShift, prim);
    } else {
        s->cursor++;
    }
    if (*s->cursor != 0) {
        prim = effectEmitGouraudTexQuads(s, ot, otShift, prim);
    } else {
        s->cursor++;
    }
    return prim;
